...
def df_to_paragraphs(df: pd.DataFrame, html_col: str = "raw_html", output_col: str = "neu_chunks"):
    ...
    slum_chunker = SlumberChunker(
        ...
        candidate_size=256,                 # How many tokens Genie looks at for potential splits
        min_characters_per_chunk=5,        # Minimum number of characters per chunk
...
# Apply to dataframe
    ...
    df["neu_chunks"] = df[html_col].progress_apply(lambda x: get_chunks(x, neu_chunker,lists=True))
    df["num_neu_chunks"] = df["neu_chunks"].apply(len)

    return df



""" 1 time modi to run slumber tiim refer back to original for other usage"""
